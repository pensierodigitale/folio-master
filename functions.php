...
if ( ! function_exists( 'folio_setup' ) ) :
/**
...
*
* Create your own folio_setup() function to override in a child theme.
*
* @since Twenty Sixteen 1.0
*/
...
function folio_setup() {
	...
	* Translations can be filed at WordPress.org. See: https://translate.wordpress.org/projects/wp-themes/foliotheme
	* If you're building a theme based on Twenty Sixteen, use a find and replace
	* to change 'foliotheme' to the name of your theme in all the template files
	*/
	load_theme_textdomain( 'foliotheme' );
	...
	*/
	...
	register_nav_menus( array(
		'primary' => __( 'Primary Menu', 'foliotheme' ),
		'social'  => __( 'Social Links Menu', 'foliotheme' ),
	) );
	/*
...
* @since Twenty Sixteen 1.0
*/
function foliotheme_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'foliotheme_content_width', 840 );
}
add_action( 'after_setup_theme', 'foliotheme_content_width', 0 );
/**
...
* @since Twenty Sixteen 1.0
*/
function foliotheme_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Sidebar', 'foliotheme' ),
		'id'            => 'sidebar-1',
		'description'   => __( 'Add widgets here to appear in your sidebar.', 'foliotheme' ),
	...
	register_sidebar( array(
		'name'          => __( 'Content Bottom 1', 'foliotheme' ),
		'id'            => 'sidebar-2',
		'description'   => __( 'Appears at the bottom of the content on posts and pages.', 'foliotheme' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
		'before_title'  => '<h2 class="widget-title">',
		'after_title'   => '</h2>',
	...
	register_sidebar( array(
		'name'          => __( 'Content Bottom 2', 'foliotheme' ),
		'id'            => 'sidebar-3',
		'description'   => __( 'Appears at the bottom of the content on posts and pages.', 'foliotheme' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
...
* Adds a `js` class to the root `<html>` element when JavaScript is detected.
	...
	* @since Twenty Sixteen 1.0
	*/
	function foliotheme_javascript_detection() {
	echo "<script>(function(html){html.className = html.className.replace(/\bno-js\b/,'js')})(document.documentElement);</script>\n";
	}
	add_action( 'wp_head', 'foliotheme_javascript_detection', 0 );
	/**
	...
	*/
	...
	wp_enqueue_script( 'menu-js', get_template_directory_uri() . '/js/menu.js', array( 'jquery' ), '20160816', true );
	...
	wp_enqueue_script( 'sidr-js', get_template_directory_uri() . '/js/jquery.sidr.min.js', array( 'jquery' ), '20160816', true );
	wp_localize_script( 'foliotheme-script', 'screenReaderText', array(
	'expand'   => __( 'expand child menu', 'foliotheme' ),
	'collapse' => __( 'collapse child menu', 'foliotheme' ),
	) );
	}
	...
	*/
	function foliotheme_body_classes( $classes ) {
	...
	add_filter( 'body_class', 'foliotheme_body_classes' );
	/**
	...
	*/
	function foliotheme_hex2rgb( $color ) {
	$color = trim( $color, '#' );
	if ( strlen( $color ) === 3 ) {
	...
	*/
	function foliotheme_content_image_sizes_attr( $sizes, $size ) {
	$width = $size[0];
	...
	add_filter( 'wp_calculate_image_sizes', 'foliotheme_content_image_sizes_attr', 10 , 2 );
	/**
	* Add custom image sizes attribute to enhance responsive image functionality
	* for post thumbnails
	*
	...
	*/
	function foliotheme_post_thumbnail_sizes_attr( $attr, $attachment, $size ) {
	...
	add_filter( 'wp_get_attachment_image_attributes', 'foliotheme_post_thumbnail_sizes_attr', 10 , 3 );
	/**
	...
	*/
	function foliotheme_widget_tag_cloud_args( $args ) {
	$args['largest'] = 1;
	$args['smallest'] = 1;
	$args['unit'] = 'em';
	return $args;
	}
	add_filter( 'widget_tag_cloud_args', 'foliotheme_widget_tag_cloud_args' );
